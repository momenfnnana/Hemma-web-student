...
export class CartItem extends Component {
    ...
    static getDerivedStateFromProps(props, state) {
        if (props.item && !props.item.canBePaidInInstallments && state.editingInstallment) {
            return {
                editingInstallment: false,
                tempInstallment: null
            };
    ...
    onToggleEditInstallment() {
        ...
        if (editing) {
            this.setState({
                ...
                tempInstallment: this.props.item.minimumInstallment / 100 || this.props.item.price / 100 || 0
            });
        } else {
    ...
    getInstallmentInputValue() {
        ...
        if (this.props.item.installment) return formatPrice(this.props.item.installment);
                    ...
                    <div className="media w-75 position-relative">
                        ...
                        <div className="media-body mt-2">
                            <h6 className="mt-0 dark-text">
                                ...
                                {item.packageOption &&
                                    <span className="smaller red-text">
                                        ( سعر الملزمة:
                                        {" "}
                                        <span className="en-text">{formatPrice(item.packageOptionPrice)}</span>
                                        {" "}
                                        ر.س. )
                                    </span>
                                }
                            ...
                            >
                                ...
                                "اعتمد القسط"
                                : item.installment ? "تعديل القسط" : "سداد بالأقساط؟"
                                }
                            </span>
                        ...
                        <div className="d-flex flex-row justify-content-between align-items-center">
                            ...
                            <div className="d-flex flex-column mx-auto">
                                <h6 className="light-text text-center mb-0">
                                    <span className="en-text">{formatPrice(item.price)}</span> ريال
                                </h6>
                                {item.priceBeforeDiscount && (
                                    <h6 className="mb-0 dark-silver-text line-through-text align-items-center d-flex">
                                        <span className="en-text">{formatPrice(item.priceBeforeDiscount)}</span>
                                        {" "}
                                        ريال
                                    </h6>
                                )}
