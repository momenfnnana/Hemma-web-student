...
export class ExpiredSubscriptions extends Component {
  page = 1;
  limit = 10;
  endOfResults = false;
  constructor(props) {
    ...
    this.state = {
      tooltipOpen: false,
      subscriptions: [],
      nextPageUrl: `${apiBaseUrl}/courses/purchased?Page=${this.page}&Limit=${this.limit}&SubscriptionStatus=Expired`
    };
  }
    ...
    if (!this.endOfResults) {
      axios
        ...
        .then(response => {
          ...
          this.page++;
          const nextUrl = `${apiBaseUrl}/courses/purchased?Page=${this.page}&Limit=${this.limit}&SubscriptionStatus=Expired`;
          this.setState({
